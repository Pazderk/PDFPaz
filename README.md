...
- **Metadata** — edit the document's Title, Author, Subject, and Keywords; takes effect on export
- **Extract Images** — downloads every embedded image in the document as a separate file (JPEG images extracted losslessly as-is; other supported formats re-encoded to PNG)
- **Compress Images** — recompresses embedded photo images on export at a chosen JPEG quality and max pixel dimension, shrinking file size while leaving text and vector content untouched; skips any image that wouldn't actually get smaller
...
- Extract Images and Compress Images only handle the common image encodings (JPEG, and raw/FlateDecode 8-bit RGB or grayscale). JPEG 2000, CCITT fax (common in older black-and-white scans), indexed-color, and other less common encodings are skipped rather than guessed at.
